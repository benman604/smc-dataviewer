...
export default function ClientPage() {
  ...
  function clearFilters() {
    setFilters({
      ...defaultFilters,
      faultTypes: [...defaultFilters.faultTypes]
    });
  }
  ...
  useEffect(() => { if (selectedEvent) setPanelOpen(true); }, [selectedEvent]);
  function closeSelectedEvent(immediate=false) {
    if (immediate) { setPanelOpen(false); setSelectedEvent(null); return; }
    setPanelOpen(false); setTimeout(() => setSelectedEvent(null), 100);
  }
  ...
  useEffect(() => {
    ...
    setView({ center: [selectedEvent.geometry.coordinates[1], selectedEvent.geometry.coordinates[0]], zoom: view.zoom });
  ...
  const renderDownload = () => (
    <Download dataFormats={SMC_EVENT_DATA_FORMATS} buildUrl={(fmt) => SMCDataURL(filters, { format: fmt })} />
  );
      ...
      <aside className="w-80 flex flex-col border-r border-stone-300">
        <div className="px-4 pt-4 pb-2 border-b border-b-stone-300">
          ...
          <div className="mt-1">
            {loading ? (
              ...
              <div className="text-xs text-red-800 flex items-center gap-2">
                ...
                <button className="text-xs text-blue-600 underline ml-2" onClick={() => fetchEvents()}>Retry</button>
              </div>
            ) : (
          ...
          {!filterOpen && (
            <div className="mt-3 grid grid-cols-4 gap-1">
              {(["Day", "Week", "Month", "Year"] as Recency[]).map((r) => (
                <button key={r} className={`rounded w-full py-2 text-sm hover:cursor-pointer ${recency === r ? 'bg-purple-600 text-white' : 'bg-stone-200 hover:bg-stone-300'}`} onClick={() => handleRecencyClick(r)}>{r}</button>
              ))}
            </div>
          )}
          ...
          <div className={`overflow-hidden transition-all duration-300 ease-in-out ${filterOpen ? 'max-h-96 translate-y-0 opacity-100' : 'max-h-0 -translate-y-3 opacity-0 pointer-events-none'}`}>
            <FilterEvents filters={filters} onChange={setFilters} onApply={() => { fetchEvents(true); setRecency(null); closeSelectedEvent(); }} />
          </div>
          ...
          <div className="mt-2 flex items-center text-s">
            ...
            <div className="flex-2 flex justify-end items-center gap-1 text-stone-600">
              ...
              <select className="hover:cursor-pointer text-xs px-1 py-0.5 border border-stone-300 rounded bg-white focus:outline-none" defaultValue="time" onChange={(e) => setOrderBy({ ...orderBy, field: e.target.value as "time" | "magnitude" })}>
              ...
              <div className="hover:cursor-pointer hover:bg-stone-200 h-6 w-6 flex items-center justify-center rounded-full border border-stone-300 bg-white text-[10px] text-stone-600 select-none">
                <FontAwesomeIcon icon={orderBy.direction === "asc" ? faArrowUpWideShort : faArrowDownWideShort} onClick={() => setOrderBy({ ...orderBy, direction: orderBy.direction === "asc" ? "desc" : "asc" })} />
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-auto p-0 m-0" role="list">
          {visibleEvents.map((event: Event, i) => (
            <div key={event.id ?? i} id={`list-event-${event.id}`} className="w-full" role="listitem">
              <div className={`px-4 py-2 border-b border-stone-300 ${selectedEvent?.id === event.id ? 'bg-purple-200 border-t-5 border-t-purple-400' : 'bg-stone-50 hover:bg-stone-100 cursor-pointer'}`} onClick={() => setSelectedEvent(event)}>
                <div className="flex gap-3 items-start">
                  <div className="shrink-0">
                    <div className="w-7 h-7 rounded-full text-xs font-bold flex items-center justify-center border border-stone-500" style={{ backgroundColor: timeToIconColor(event.properties.time) }}>
                      {event.properties.mag?.toFixed(1) ?? "-"}
                    </div>
                  </div>
                  <div className="flex-1">
                    <p className="leading-tight">{event.properties.title}</p>
                    <p className="text-xs text-stone-500">{event.properties.place}, {parseImplicitUTCToLocal(event.properties.time).toLocaleString()}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <main className="flex-1 min-h-0">
        <section className="h-full min-h-0 relative overflow-hidden">
          ...
          <div className={`absolute top-4 w-80 p-4 bg-white border border-stone-300 transition-all duration-300 ease-in-out ${panelOpen ? 'right-4 opacity-100 pointer-events-auto' : 'right-[-22rem] opacity-0 pointer-events-none'}`} style={{ zIndex: 100000 }} aria-hidden={!panelOpen}>
            <button className="flex items-center justify-center pt-0 mt-0 mb-2 text-blue-500 hover:text-blue-700 hover:cursor-pointer" onClick={() => closeSelectedEvent()} aria-label="Close">
            ...
            {selectedEvent && (
              <>
                ...
                <p className="mt-2 flex flex-wrap gap-2 text-sm text-stone-600">
                  ...
                  {selectedEvent.properties.faultType && <span className="inline-flex items-center gap-1 rounded-full bg-stone-100 px-2 py-0.5">{faultIdToName(selectedEvent.properties.faultType)}<span className="font-medium text-stone-700">Fault</span></span>}
                  <span className="inline-flex items-center gap-1 rounded-full bg-stone-100 px-2 py-0.5">{selectedEvent.id}</span>
                </p>

                <div className="flex flex-wrap justify-center gap-2">
                  {selectedEvent.properties.RecordNum > 0 && (
                    <Link href={`/events/records?evid=${selectedEvent.id}${window.location.search ? '&from=' + encodeURIComponent(window.location.search) : ''}`} className="flex flex-col items-center py-1 px-2 opacity-70 hover:opacity-100 bg-white cursor-pointer" title="Interactive Map">
                  ...
                  {CISNShakemapURL(selectedEvent) && (
                    <a className="flex flex-col items-center py-1 px-2 opacity-70 hover:opacity-100 bg-white cursor-pointer" title="ShakeMap" href={CISNShakemapURL(selectedEvent)} target="_blank" rel="noopener noreferrer">
